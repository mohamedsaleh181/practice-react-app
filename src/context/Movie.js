...
const  MovieContextProvider = ({children})=>{
    const initialFavList = JSON.parse(localStorage.getItem("favList"))
    const initialFavIds = JSON.parse(localStorage.getItem("favIds"))
    ...
    const removeFav = (movie)=>{
        let newFavList = favList.filter((favMovie)=>{
            return favMovie.id !== movie.id
        })
        setFavList(newFavList)
        let newFavIds = favIds.filter((favId)=>{
            return favId !== movie.id
        })
        setFavIds(newFavIds)
    }
    //function to add to favorite list
    const addToFav = (movie)=>{
        if(!favIds.includes(movie.id)){
            
                let newFavList = [...favList, movie]
                setFavList([...newFavList])
                setFavIds([...favIds, movie.id])
    ...
    return <movieContext.Provider value={{movies, addToFav, favList, removeFav, favIds}}>
            {children}
